...
export default function ControlPanelPage() {
  ...
  const [callStatus, setCallStatus] = useState('กำลังโทร...');
  ...
  function startCall(c: CallState) {
    ...
    setTimeout(() => {
      setCallStatus(c.kind === 'emergency' ? '● กำลังประกาศไซเรน' : '● กำลังประกาศ');
  ...
  function sendTTS() {
    const text = ttsText.trim() || 'ทดสอบระบบ Text-to-Speech';
    setShowTTS(false);
  ...
  const selectedNames = useMemo(() => {
    ...
    if (ids.length === 0) return 'ยังไม่ได้เลือกจุดใด';
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-800">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 py-6 space-y-6">
        ...
        <TemplateStrip
          onPick={t => startCall({ kind: 'template', speakers: projectSpeakers, template: t })}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_320px] gap-6">
          <section className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-3">
            {filteredSpeakers.length === 0 && (
              <div className="col-span-full text-center text-slate-400 py-12">ไม่พบจุดประกาศตามเงื่อนไข</div>
            )}
