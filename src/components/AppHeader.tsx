...
const NAV: NavItem[] = [
  { href: '/app', label: 'แผงควบคุม', flag: 'op' },
  { href: '/mp3', label: 'ไฟล์เสียง', flag: 'mp3' },
  { href: '/scheduler', label: 'ตั้งเวลา', flag: 'scheduler' },
  { href: '/log', label: 'บันทึก', flag: 'log' },
  { href: '/status', label: 'สถานะระบบ', flag: 'status' },
  { href: '/users', label: 'ผู้ใช้', flag: 'users' },
];
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        <div className="flex items-center justify-between h-16">
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="font-bold text-slate-900 leading-tight">ระบบประกาศเสียงไร้สาย</div>
              <div className="text-xs text-slate-500 truncate max-w-[180px] sm:max-w-none">
                {project ? `โครงการ: ${project.name}` : '—'}
              </div>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3">
            <div className="hidden sm:flex items-center gap-2 px-3 py-1.5 bg-green-50 text-green-700 rounded-full text-xs font-medium border border-green-200">
              <span className="pulse-dot"></span> เชื่อมต่อแล้ว
            </div>
            <div className="hidden sm:block text-right">
              <div className="text-sm font-medium text-slate-800">{user ? user.name : '—'}</div>
              <span className={`inline-flex px-1.5 py-0.5 text-[10px] font-semibold rounded ring-1 ${roleBadgeClass(role)}`}>
                {r ? r.name : role}
              </span>
            </div>
            <Link href="/" className="text-sm text-slate-500 hover:text-red-600" title="ออกจากระบบ">
              <svg xmlns="http://www.w3.org/2000/svg" className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
