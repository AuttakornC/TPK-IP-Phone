...
interface Props {
  /** When provided, only speakers belonging to this project are shown. Omit/null = all (admin scope). */
  projectId?: string | null;
}

export default function StatusContent({ projectId = null }: Props = {}) {
  ...
  const visibleSpeakers = projectId ? SPEAKERS.filter(s => s.projectId === projectId) : SPEAKERS;
  const visibleOnline = visibleSpeakers.filter(s => s.online).length;
    ...
    <div className="space-y-6">
      ...
      <section className="bg-white border border-slate-200 rounded-2xl overflow-hidden">
        <div className="px-5 py-4 border-b border-slate-200 flex items-center justify-between">
          ...
          <span className="text-xs text-slate-500">{t('speakerHealthCount', { online: visibleOnline, total: visibleSpeakers.length })}</span>
        </div>
          ...
          {visibleSpeakers.map(s => (
            <div key={s.id} className={`flex items-center gap-3 px-3 py-2 rounded-lg border ${s.online ? 'border-slate-200' : 'border-red-200 bg-red-50'}`}>
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="text-xs text-slate-500">{t('speakerExt', { ext: s.ext, area: s.area })}</div>
              </div>
