...
const TIER_LABEL = Object.fromEntries(PACKAGE_TIERS.map(t => [t.id, t]));

// ============= SPEAKERS (each belongs to a project) =============
const SPEAKERS = [
  // Project p1 — เทศบาลตำบลบางสะพาน (14 speakers)
  ...
  // Project p2 — อบต.ห้วยกระเจา (8 speakers)
  ...
  // Project p3 — เทศบาลเมืองฉะเชิงเทรา (5 speakers)
...
// ============= USERS (across all projects + admin vendor) =============
const USERS = [
  // Admin (vendor) — no project
  ...
  // Project p3 — เทศบาลเมืองฉะเชิงเทรา
...
// Demo presets — index.html role selector picks one user per role
const DEMO_USER_BY_ROLE = {
  admin:       'admin',
  authority:   'somphong', // project p1
  officer:     'somchai',  // project p1
  headVillage: 'manit',    // project p1, has 3 speakers (incl. 1 offline)
};
...
const ROLE_LABEL = Object.fromEntries(ROLES.map(r => [r.id, r]));
...
const SYSTEM_STATUS = {
  asterisk:  { status: 'ok', uptime: '14 วัน 3 ชม.', cpu: 12, mem: 38, version: '20.4.0' },
  webApp:    { status: 'ok', uptime: '2 วัน 8 ชม.',  cpu: 5,  mem: 22, version: '1.0.0-beta' },
  database:  { status: 'ok', uptime: '14 วัน 3 ชม.', size: '184 MB' },
  storage:   { used: 4.2, total: 50, unit: 'GB', logRetention: 90, recordingRetention: 90 },
  network:   { latency: 12, lossPercent: 0, wssOnline: true },
  speakers:  { online: 12, total: 14, lastCheck: '2 นาทีที่แล้ว' }, // current project (p1)
  globalSpeakers: { online: 28, total: 33 },
  backup:    { last: '2026-05-07 02:00', size: '4.1 GB', status: 'ok', next: '2026-05-08 02:00' },
  ssl:       { issuer: "Let's Encrypt", validUntil: '2026-07-15', daysLeft: 69 },
  recentIssues: [
    { time: '2026-05-07 03:14', severity: 'warning', message: 'วัดเหนือ (p1) ไม่ตอบสนอง > 5 นาที' },
    { time: '2026-05-06 22:08', severity: 'info',    message: 'หอนาฬิกา (p1) กลับมา online' },
    { time: '2026-05-06 22:03', severity: 'warning', message: 'หอนาฬิกา (p1) offline' },
    { time: '2026-05-05 02:00', severity: 'info',    message: 'Backup สำเร็จ (4.1 GB)' },
    { time: '2026-05-04 14:22', severity: 'info',    message: 'อัปเดตระบบ Web App เป็น 1.0.0-beta' },
  ],
};

const ZONE_LABEL = Object.fromEntries(ZONES.map(z => [z.id, z.name]));
